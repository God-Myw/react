...
class ShipSparePartsView extends React.Component<ShipSparePartsViewFormProps> {
      ...
      if (response.status === 200 && !isNil(response)) {
        ...
        if (data.spartParts) {
          data.spartParts.map((v: any, i: any) => {
            const storeObj: { index?: String, imgUrl?: any, type?: any, price?: any, count?: any } = {
              index: i,
              imgUrl:
                <Upload
                  ...
                  onChange={(info: any) => {
                    ...
                    const spartParts2: any = this.state.spartParts.filter((item: any, i: any) => i == i)
              ...
              type:
                <Form.Item required>
                  <Input defaultValue={v.model} onChange={e => {
                    this.storeListChange(storeObj.index, 'model', e.target.value)
                  }} ></Input>
                </Form.Item>,
              price:
                <Form.Item required>
                  <Input defaultValue={v.spartMoney} onChange={e => {
                    this.storeListChange(storeObj.index, 'spartMoney', e.target.value)
                  }} ></Input>
                </Form.Item>,
              count:
                <Form.Item required>
                  <Input defaultValue={v.quantity} onChange={e => {
        ...
        this.setState({
          ...
          money: data.money || '',
          number: data.number || '',
          partExplain: this.partExplain([...new Set(data.partExplain.split('/'))].join('/')) || '',
  ...
  storeListChange = (index: any, type: string, value: any) => {
    const spartParts: { index?: number, partPicList: any, model?: any, spartMoney?: any, quantity?: any }[] = [...this.state.spartParts]
      ...
      imgUrl:
        <Upload
          ...
          onChange={(info: any) => {
            ...
            const spartParts: any[] = [...this.state.spartParts]
            const spartParts2: any[] = this.state.spartParts.filter((item: any, i: any) => i == index)
            let spartPartsInit: { index?: any, partPicList?: any, model: any, spartMoney: any, quantity: any } = {
              index: index,
              partPicList: dataSource,
              model: spartParts2[0].model,
              spartMoney: spartParts2[0].spartMoney,
  ...
  partExplain = (e: any) => {
    return (
      e.split('/').filter(Boolean).map((v: any, i: number) => {
        return (<Input
          key={i}
          defaultValue={v}
          onBlur={(e) => {
  ...
  getShipSparePartsEdit = () => {
    let partExplain: any = [...new Set(this.state.partExplain.map((item: any) => {
      return item.props.defaultValue
    }))].join('/')
    let requestData = {
              ...
              <Row gutter={24}>
                <Col span={12}>
                  <Form.Item required {...formlayout} label="商品轮播图">
                    <Input style={{ width: '73%' }} defaultValue={'请选择上传图片'} disabled></Input>
                  </Form.Item>
                  <Upload
                    action={"/api/sys/file/upLoadFuJian/" + fileType.ship_spare}
                    listType="picture-card"
                ...
                <Row className={commonCss.rowTop}>
                  <Col span={12} pull={1} className={commonCss.lastButtonAlignRight}>
                    <ButtonOptionComponent
                      ...
                      type="Save"
                      text={'取消'}
                      event={() => { this.props.history.push('/spartPart') }}
                    />
                  </Col>
                  <Col span={12}>
                    <ButtonOptionComponent
...
const MPCertificationList_Form = Form.create({ name: 'MPCertificationList_Form' })(ShipSparePartsView);

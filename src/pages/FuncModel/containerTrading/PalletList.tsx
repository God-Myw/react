...
class PalletListForm extends React.Component<RouteComponentProps> {
  ...
  setParams(): Map<string, string> {
    ...
    params.set('portName', this.state.startDate ? this.state.startDate : '');
    params.set('accountId', this.state.user ? this.state.user : '');
    return params;
  }
  ...
  getPalletList() {
    ...
    getRequest('/business/shipBooking/getContainerTradingUserForWeb', param, (response: any) => {
      ...
      if (response.status === 200) {
        if (!isNil(response.data)) {
          forEach(response.data.result, (pallet, index) => {
            ...
            entity.key = index + 1;
            entity.orderNumber = pallet.detail[0] ? pallet.detail[0].orderNumber : '';


            entity.namess = pallet.portName;

            entity.startDate = pallet.detail[0] ? pallet.detail[0].size + '（' + (pallet.detail[0].type === 0 ? '全新' + '）' + pallet.detail[0].count + '个' : '二手' + '）' + pallet.detail[0].count + '个') : '';

            entity.endDate = pallet.detail[1] ? pallet.detail[1].size + '（' + (pallet.detail[1].type === 0 ? '全新' + '）' + pallet.detail[1].count + '个' : '二手' + '）' + pallet.detail[1].count + '个') : '';
            entity.haiyunMoneyOneOld = pallet.detail[2] ? pallet.detail[2].size + '（' + (pallet.detail[2].type === 0 ? '全新' + '）' + pallet.detail[2].count + '个' : '二手' + '）' + pallet.detail[2].count + '个') : '';
            entity.haiyunMoneyTwoOld = pallet.detail[3] ? pallet.detail[3].size + '（' + (pallet.detail[3].type === 0 ? '全新' + '）' + pallet.detail[3].count + '个' : '二手' + '）' + pallet.detail[3].count + '个') : '';
  ...
  getPalletList_ss() {
    ...
    param.set('portName', this.state.startDate ? this.state.startDate : '');
    param.set('accountId', this.state.user ? this.state.user : '');
  ...
  //货盘查看
  handleView = (guid: any) => {
    this.props.history.push('/containerTrading/view/' + guid.guid);
  };

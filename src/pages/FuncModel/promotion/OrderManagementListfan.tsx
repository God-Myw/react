...
class AdvanceorderListForm extends React.Component<RouteComponentProps> {
  ...
  getsource() {
    ...
    param.set('auditStatus', this.state.auditStatus);//订单状态
    param.set('orderNumber', this.state.orderNumber);//订单编号
    param.set('createDate', this.state.createDate);//订单日期
    param.set('prStart', this.state.prStart);//出发地
    param.set('prEnd', this.state.prEnd);//目的地
  ...
  render() {
    return (
      <div className={commonCss.container}>
        <LabelTitleComponent text="加油返利订单" event={() => this.onBack()} />
            ...
            <Col span={24}>
              <InputGroup compact>
                ...
                <Select defaultValue="请选择支付状态查询" allowClear={true} style={{ width: '14%' }} onChange={this.zhifuzhuangtai}>
                  ...
                  <Option value="1">待确认订金支付</Option>
                  <Option value="2">待确认尾款支付</Option>

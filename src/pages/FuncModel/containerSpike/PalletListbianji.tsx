...
class PalletListForm extends React.Component<RouteComponentProps> {
  ...
  setParams(): Map<string, string> {
    ...
    params.set('orderNumber',this.state.orderNumber?this.state.orderNumber:'');//订单编号
    params.set('phoneCode',this.state.phoneCode?this.state.phoneCode:'');//手机号
    params.set('orderStatus',this.state.orderStatus?this.state.orderStatus:'');//状态
    return params;
  }
  ...
  getPalletList() {
    ...
    getRequest('/business/containerSpike/getSpikeListForWeb', param, (response: any) => {
      ...
      if (response.status === 200) {
        if (!isNil(response.data)) {
          ...
          forEach(response.data.result, (pallet, index) => {
            ...
            entity.country = pallet.startPortCn+'---'+pallet.endPortCn;

            entity.closingTime = pallet.closingTime;

            entity.boxs1 = pallet.boxs[1]?(pallet.boxs[1].boxType+'*'+pallet.boxs[1].stock):'';
            entity.boxs2 = pallet.boxs[2]?(pallet.boxs[2].boxType+'*'+pallet.boxs[2].stock):'';
            entity.boxs3 = pallet.boxs[3]?(pallet.boxs[3].boxType+'*'+pallet.boxs[3].stock):'';

            entity.bond = '￥'+pallet.bond;

            entity.phoneNumber = pallet.phoneNumber;

            entity.startDate = pallet.startDate;

            entity.endDate = pallet.endDate;

            entity.count = pallet.count;
  ...
  handleView = (guid: any, e: any) => {
    ...
    console.log(e)
    this.props.history.push('/containerSpike/editBia/' + guid.guid+'/' + e);
  };
          ...
          <Row gutter={24}>
            <Col span={24}>
              ...
              <Input
                ...
                <QueryButton key={3} type="Query"  text="搜索" event={() => this.initData(999)} disabled={false} />

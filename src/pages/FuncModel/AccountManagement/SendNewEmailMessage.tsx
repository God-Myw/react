    ...
    this.props.form.validateFields((err: any, values: any) => {
      if (!err) {
        ...
        putRequest('/sys/user', JSON.stringify(requestData), (response: any) => {
          if (response.status === 200) {
            this.props.history.push('/account_manager/newEmailSuccess');
            message.success(formatMessage({ id: 'AccountManagement-SendNewEmailMessage.successful' }))
          } else {
            message.error(formatMessage({ id: 'AccountManagement-ModifyPassword.vertify.error.again' }));
          }
        });
